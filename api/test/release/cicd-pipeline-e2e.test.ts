/**
 * CI/CD Pipeline Mode - E2E Test
 * 
 * This test simulates a complete release workflow with:
 * - hasManualBuildUpload = false (CI/CD pipeline mode)
 * - 2 platforms: IOS + ANDROID
 * - 2 regression slots
 * - Automatic stage transitions
 * - CI/CD build triggers (mocked)
 * 
 * Run with: npx ts-node -r tsconfig-paths/register test/release/cicd-pipeline-e2e.test.ts
 */
...
const TEST_CONFIG = {
  ...
  VERSION: '8.0.0',
  // CI/CD PIPELINE MODE (NO MANUAL UPLOADS)
...
async function createTestTenant(
  ...
  await AccountModel.findOrCreate({
    ...
    defaults: {
      id: accountId,
      email: `cicd-test-${uuidv4()}@example.com`,
      name: 'Test User - CI/CD Pipeline',
    } as any
  });
  ...
  await TenantModel.findOrCreate({
    ...
    defaults: {
      id: tenantId,
      displayName: 'Test Tenant - CI/CD Pipeline',
      createdBy: accountId,
    } as any
  });
}
...
async function createTestRelease(
  ...
  return releaseRepo.create({
    ...
    releaseDate: null,
    hasManualBuildUpload: options.hasManualBuildUpload, // ❌ CI/CD mode!
...
async function printDatabaseState(sequelize: Sequelize, releaseId: string) {
  console.log('\n' + '='.repeat(80));
  console.log('📦 DATABASE STATE - CI/CD Pipeline Test');
...
async function runCICDPipelineE2ETest() {
  console.log('\n' + '='.repeat(80));
  console.log('🚀 CI/CD PIPELINE MODE - E2E TEST');
  console.log('='.repeat(80));
  ...
  const release = await createTestRelease(releaseRepo, {
    ...
    hasManualBuildUpload: TEST_CONFIG.HAS_MANUAL_BUILD_UPLOAD // ❌ CI/CD mode!
  });
  ...
  // -------------------------------------------------------------------------
  // CI/CD MODE: No manual uploads - builds will be triggered via CI/CD pipeline
  // -------------------------------------------------------------------------
  log('\n📝 STEP 5 [CI/CD Mode]: Skipping manual uploads - using CI/CD pipeline...');
  log('  ✅ Builds will be triggered automatically via mocked CI/CD workflows');
  ...
  // CI/CD mode - no manual uploads needed
  ...
  while (iteration < TEST_CONFIG.MAX_ITERATIONS) {
    ...
    try {
      ...
      const stateMachine = new CronJobStateMachine(
        ...
        releaseUploadsRepo     // Optional - not used in CI/CD mode
      );
  ...
  log(`\n📋 Release Uploads (CI/CD mode - should be empty):`);
  ...
  log('\n' + '='.repeat(60));
  if (success) {
    log(`🏁 CI/CD PIPELINE TEST PASSED ✅ in ${minutes}m ${seconds}s`);
  } else {
    log(`🏁 CI/CD PIPELINE TEST INCOMPLETE ⚠️ in ${minutes}m ${seconds}s`);
...
// Run the test
runCICDPipelineE2ETest().catch(console.error);

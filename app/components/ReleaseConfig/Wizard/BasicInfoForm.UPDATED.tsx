/**
 * Basic Information Form - UPDATED VERSION
 * Now uses ConfigContext for dynamic release types
 */
...
export function BasicInfoForm({ config, onChange, tenantId }: BasicInfoFormProps) {
  ...
  // Get release types from backend
  const allReleaseTypes = getReleaseTypes();
  
  // Filter to only allowed types for this tenant
  const allowedReleaseTypes = allReleaseTypes.filter(rt =>
  ...
  return (
    <div className="space-y-4">
      ...
      <Select
        ...
        disabled={allowedReleaseTypes.length === 0}
      />
